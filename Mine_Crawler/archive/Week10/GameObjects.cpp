...
void Character::attackTarget(Level* pMap) // possibly reduntant
{
	std::cout << "attacking";
	...
	if (!isBusy)
	{
		if (nextMoveTime < SDL_GetTicks())
		{
			...
			else // we are not close
			{
				...
				float angleToTarget = atan2(dY, dX); // this is in radians

				// Calcuate next X & Y				
				nextX = currentX + round(1.4 * cos(angleToTarget));
				nextY = currentY + round(1.4 * sin(angleToTarget));
			}
