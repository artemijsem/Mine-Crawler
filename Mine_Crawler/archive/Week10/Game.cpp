...
void Game::createGameObjects()
{
	...
	squadYellow->createUnits(map, 10, 2); // level, spawn amount & state

	// Red Squad - Player PCs
	squadRed = new Squad("Red Team", 6, 30, 5); // name, tileType, start x y 
	squadRed->createUnits(map, 5, 0);
	...
	{
		highLights[i] = new Item("assets/images/square_cross_yellow.png", -1, -1, 8);
	}
	...
	{
		bulletsRed[i] = new Attack("assets/images/Pointer.png", 300, 300, 16);
	}

	for (int i = 0; i < sizeof(hitMarkers) / sizeof(hitMarkers[0]); i++)
	{
		hitMarkers[i] = new Item("assets/images/Star_Red.png", -1, -1, 8);
	}
...
void Game::findNearestTarget() // Only for Red Units 
{
	...
	for (int redUnit : redUnitsActive)
	{
		...
		squadRed->setUnitTarget(redUnit - 601, nearestTarget);
			...
			if (objectColldingWith > 0) // bullet hit something
			{
				...
				if (objectColldingWith == 5) // Yellow Unit Hit
				{
					...
					for (int unit : yelActiveUnits)
					{
						...
						{
							// Loop through inactive hitmarkers
							for (int j = 0; j < sizeof(hitMarkers) / sizeof(hitMarkers[0]); j++)
							{
								if (!hitMarkers[j]->getIsActive())
								{
									hitMarkers[j]->isActive = true;

									int spawnX = yellowUnitPos.x * map->getMapTileSize() + map->getStartX();
									int spawnY = yellowUnitPos.y * map->getMapTileSize() + map->getStartY();

									hitMarkers[j]->x = spawnX;
									hitMarkers[j]->y = spawnY;
...
{ // Move Objectct as the map moves
	...
	for (int i = 0; i < sizeof(hitMarkers) / sizeof(hitMarkers[0]); i++)
	{
		if (hitMarkers[i]->getIsActive())
		{
			hitMarkers[i]->x += pMoveAmountX;
			hitMarkers[i]->y += pMoveAmountY;
		}
	}
}//---
...
void Game::updateGameObjects()
{
	...
	//--------------------------------------------------------------------------
	// Even slower update
	if (evenSlowerUpdateTime < SDL_GetTicks64())
	{
		for (int i = 0; i < sizeof(hitMarkers) / sizeof(hitMarkers[0]); i++)
		{
			if (hitMarkers[i]->getIsActive()) hitMarkers[i]->isActive = false;
		}

		evenSlowerUpdateTime = SDL_GetTicks64() + 100;
	}
...
void Game::render()
{
	...
	for (int i = 0; i < sizeof(bulletsRed) / sizeof(bulletsRed[0]); i++)
	{
		if (bulletsRed[i]->getIsActive()) 	bulletsRed[i]->renderEX();
	}
	...
	for (int i = 0; i < sizeof(hitMarkers) / sizeof(hitMarkers[0]); i++)
	{
		if (hitMarkers[i]->getIsActive()) hitMarkers[i]->render();
	}
